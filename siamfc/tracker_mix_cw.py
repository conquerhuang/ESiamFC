...
class SiamFCTrackerMixCw:
    def __init__(self, model_path, gpu_id, is_deterministic=False, hp_config=None):

        self.gpu_id = gpu_id
        self.name = 'SiamFC_Mix_Cw'
        self.is_deterministic = is_deterministic  # 暂时不用考虑他的用处

        with torch.cuda.device(gpu_id):
            self.model = SiameseAlexNetMixCw(gpu_id, train=False)  # 加载模型
        ...
        # 用于超参数调整
        self.hp_config = edict()
        if hp_config is None:
            self.hp_config.window_influence = config.window_influence
            self.hp_config.instance_size = config.instance_size
            self.hp_config.scale_step = config.scale_step
            self.hp_config.scale_lr = config.scale_lr
            self.hp_config.scale_penalty = config.scale_penalty
        else:
            self.hp_config.window_influence = hp_config.window_influence
            self.hp_config.instance_size = hp_config.instance_size
            self.hp_config.scale_step = hp_config.scale_step
            self.hp_config.scale_lr = hp_config.scale_lr
            self.hp_config.scale_penalty = hp_config.scale_penalty
    ...
    def init(self, frame, box):
        ...
        self.penalty = np.ones((config.num_scale)) * self.hp_config.scale_penalty
        ...
        self.scales = self.hp_config.scale_step ** np.arange(np.ceil(config.num_scale / 2) - config.num_scale,
                                                     np.floor(config.num_scale / 2) + 1)

        # create s_x  ？ instance是exemplar的2倍
        self.s_x = s_z + (self.hp_config.instance_size - config.exemplar_size) / scale_z  # s-x原始 “搜索区域”的大小；s-z是原始模板的大小
    ...
    def update(self, frame):
        ...
        pyramid = get_pyramid_instance_image(frame, self.pos, self.hp_config.instance_size, size_x_scales, self.img_mean)
        ...
        response_map = response_map / (response_map.sum() + 1e-9)
        response_map = (1 - self.hp_config.window_influence) * response_map + \
                       self.hp_config.window_influence * self.cosine_window  # 添加余弦窗
        ...
        disp_response_frame = disp_response_input * (self.s_x * scale) / self.hp_config.instance_size  # 真实的相对位移
        # position in frame coordinates
        self.pos += disp_response_frame
        # scale damping and saturation #比例阻尼和饱和度
        self.s_x *= ((1 - self.hp_config.scale_lr) + self.hp_config.scale_lr * scale)  # 尺度更新？？？
        self.s_x = max(self.min_s_x, min(self.max_s_x, self.s_x))
        self.target_sz = ((1 - self.hp_config.scale_lr) + self.hp_config.scale_lr * scale) * self.target_sz  # 尺度更新
