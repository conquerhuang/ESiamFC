...
class SiamFCTrackerMix:
    ...
    def update(self, frame):
        ...
        with torch.cuda.device(self.gpu_id):
            ...
            response_maps_list = self.model((None, instance_imgs_var))  # 3,1,17,17， 因为是三种尺度
            # 这里生成的 response_maps_list 是一个大小为6的list.其中每一个list对应一个簇分支在三个尺度下的响应图。后续代码，需要按照我们
            # 的融合方法，将这些跟踪器上得到的结果相融合。

            # ① 简单的融合方法，直接相加得到响应图
            response_maps = None
            for branch_response_map in response_maps_list:
                if response_maps==None:
                    response_maps = branch_response_map/6.
                else:
                    response_maps += branch_response_map/6.
        ...
        box = self.branch_update(response_maps_up)
        return box
    ...
    def branch_update(self, response_maps_up):
        ...
        # return scale, disp_response_frame,  box
        return box
