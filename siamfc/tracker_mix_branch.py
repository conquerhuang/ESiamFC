...
class SiamFCTrackerMixBranch:
    def __init__(self, model_path, gpu_id, is_deterministic=False, sigma=3, mu=2, gamma=1.4):

        # 调试用超参数
        self.gamma = gamma
        self.sigma = sigma # 调试用超参数
        self.mu = mu

        # 跟踪器相关参数
        self.gpu_id = gpu_id
        self.name = 'SiamFC_sep'
        self.is_deterministic = is_deterministic  # 暂时不用考虑他的用处

        with torch.cuda.device(gpu_id):
            self.model = SiameseAlexNetMix(gpu_id, train=False)  # 加载模型
    ...
    def update(self, frame):
        ...
        with torch.cuda.device(self.gpu_id):
            ...
            response_maps_allbranch = self.model((None, instance_imgs_var))  # 3,1,17,17， 因为是三种尺度
            # 这里生成的 response_maps_list 是一个大小为6的list.其中每一个list对应一个簇分支在三个尺度下的响应图。后续代码，需要按照我们
            # 的融合方法，将这些跟踪器上得到的结果相融合。

        # 得到在每一个分支跟踪器对应的响应图上，目标的位置，尺度，PSR,APCE等各项参数**********************************
        branch_scales = []
        branch_disp_response = []
        branch_psr = []
        for response_maps in response_maps_allbranch:
            ...
            scale, disp_response, psr = self.branch_update(response_maps_up)
            branch_scales.append(scale)
            branch_disp_response.append(disp_response)
            branch_psr.append(psr)

        # 根据上一步骤得到的相关参数，计算每个分支跟踪器对应的权重**************************************************
        branch_weight = np.zeros(6)
        for i in range(6):
            scale, disp_response, psr = branch_scales[i], branch_disp_response[i], branch_psr[i]
            disp_weight = 1./((disp_response**2 / self.target_sz) + self.gamma)
            psr_weight = 1./(1 + np.exp(-self.gamma * (psr - self.mu)))
            branch_weight[i] = psr_weight
        branch_weight = np.exp(branch_weight) / np.sum(np.exp(branch_weight))

        # 根据上一步得到的权重，选择某一特定跟踪器，或者进行结果融合。**********************************************
        scale_mean = None
        disp_response_mean = None
        for i in range(6):
            scale, disp_response, weight = branch_scales[i], branch_disp_response[i], branch_weight[i]
            if scale_mean == None:
                scale_mean = scale * weight
                disp_response_mean = disp_response * weight
            else:
                scale_mean += scale * weight
                disp_response_mean += disp_response * weight

        # 更新self.pos, self.s_x, self.target_sz**********************************************************
        self.pos += disp_response_mean
    ...
    def branch_update(self, response_maps_up):
        # -------------------进行尺度搜索------------------
        ...
        # print('response map sum:{:.3f}'.format(response_map.sum()))
        response_map /= 2e3
        ...
        # 计算响应图的PSR （裁剪目标对象位置区域的图像）
        # 从响应图中，计算得到的最大值的位置，裁剪出135*135大小的区域，计算该区域的PSR。
        x_l, x_r, y_d, y_u = max(max_r - 67, 0), min(max_r+68, 271), max(max_c-67, 0), min(max_c+68, 271)
        target_response_map = response_map[x_l:x_r, y_d:y_u]
        y = np.arange(x_l-max_r,x_r-max_r,1)
        x = np.arange(y_d-max_c, y_u-max_c,1)
        x, y = np.meshgrid(x, y)
        dist = np.sqrt(x**2 + y**2)

        peak_mask = np.ones(target_response_map.shape)  # 计算中心部分的掩膜
        if target_response_map.shape != dist.shape:
            a = []
        peak_mask[dist<=3] = 0      # 35 default
        peak_mask_inv = 1 - peak_mask
        target_response_map = target_response_map * peak_mask #  将中心部分去掉，方便后续过程中计算峰值旁瓣比。
        g_max = response_map[max_r, max_c]   # 计算最大峰值
        miu = target_response_map.sum() / peak_mask.sum()
        target_response_map += peak_mask_inv * miu
        sigma = target_response_map.std() * ((peak_mask.size-1)/(peak_mask.sum()-1))
        psr = (g_max - miu)/sigma

        # # 计算响应图PSR(整个响应图区域)
        # y = np.arange(-max_r, -max_r + 272, 1)
        # x = np.arange(-max_c, -max_c + 272, 1)
        # x, y = np.meshgrid(x, y)
        # dist = np.sqrt(x**2+y**2)
        # peak_mask = np.ones(response_map.shape)
        # peak_mask[dist<=35] = 0
        # peak_mask_inv = 1 - peak_mask
        # target_response_map = response_map * peak_mask
        # g_max = response_map[max_r, max_c]
        # miu = target_response_map.sum() / peak_mask.sum()
        # target_response_map += peak_mask_inv * miu
        # sigma = target_response_map.std() * ((peak_mask.size - 1) / (peak_mask.sum() - 1))
        # psr = (g_max - miu) / sigma
        ...
        return scale, disp_response_frame, psr
