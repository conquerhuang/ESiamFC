...
from sklearn.model_selection import train_test_split  # 用于机器学习的一个库，里面包含了机器学习的相关数据。
...
def train_sep(gpu_id, data_dir, meta_data_path, model_path):
    '''
    data_dir: 训练数据对应的lmdb文件所在路径
    meta_data_path:训练数据对应的元数据目录所在的路径
    model_path: 存放模型的路径
    '''
    # loading meta data
    ...
    with torch.cuda.device(gpu_id):
        model = SiameseAlexNetSep(gpu_id, train=True, model_path=model_path)  # __init__
        ...
        for epoch in range(config.epoch):  # epoch=30
            ...
            for i, data in enumerate(tqdm(trainloader)):
                ...
                exemplar_var, instance_var = Variable(exemplar_imgs.cuda()), Variable(
                    instance_imgs.cuda())  # 这两个必须是Variable吗
            ...
            torch.save(
                ...
                "../models/siamfc_cluster{}_{}.pth".format(
                    meta_data_path.split('cluster_result/cluster')[1].split('_meta_data')[0],
                    epoch + 1
                )
            )  # 把model转换到cpu上进行保存
