...
def train_mix_cw(gpu_id, data_dir, model_path):
    # loading meta data
    ...
    with torch.cuda.device(gpu_id):
        model = SiameseAlexNetMixCw(gpu_id, train=True, model_path=model_path)  # __init__
        ...
        for epoch in range(config.epoch):  # epoch=30
            ...
            torch.save(model.cpu().state_dict(), "../models/siamfc_mix_cw{}.pth".format(epoch + 1))  # 把model转换到cpu上进行保存
