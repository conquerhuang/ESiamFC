...
from siamfc import SiameseFeature1, SiameseFeature2, ClusterWeight

from siamfc.config import config


class SiameseAlexNetMixCw(nn.Module):
    def __init__(self, gpu_id, train=True, model_path=''):
        super(SiameseAlexNetMixCw, self).__init__()
        ...
        # 加载各个分支的偏置
        self.corr_bias1 = nn.Parameter(torch.zeros(1), requires_grad=False)  # Parameter
        self.corr_bias2 = nn.Parameter(torch.zeros(1), requires_grad=False)
        self.corr_bias3 = nn.Parameter(torch.zeros(1), requires_grad=False)
        self.corr_bias4 = nn.Parameter(torch.zeros(1), requires_grad=False)
        self.corr_bias5 = nn.Parameter(torch.zeros(1), requires_grad=False)
        self.corr_bias6 = nn.Parameter(torch.zeros(1), requires_grad=False)

        # 加载分支权重模型
        self.cluster_weight = ClusterWeight()
        ...
        self.gpu_id = gpu_id
        self.clu_weight = None  # 簇分支权重

    def features_exemplar(self, x):
        x = self.stem(x)
        self.clu_weight = self.cluster_weight(x)  # 通过簇分支权重模块，计算簇分支权重
        out1 = self.branch1(x)
        out2 = self.branch2(x)
        out3 = self.branch3(x)
        out4 = self.branch4(x)
    ...
    def init_weights(self):
        for m in self.cluster_weight.modules():
    ...
    def forward(self, x):
        ...
        if exemplar is not None and instance is not None:  #
            ...
            exemplars = self.features_exemplar(exemplar)  # 8 ，256， 6 ， 6
            instances = self.features_instance(instance)  # 8,  256， 20, 20

            scores = None
            for i in range(len(exemplars)):
                exemplar, instance, corr_bias = exemplars[i], instances[i], self.corr_biass[i]
                ...
                score = score * self.clu_weight[:, i:i+1].unsqueeze(dim=2).unsqueeze(dim=2)
                if i == 0:
                    scores = score
                else:
                    scores += score
        ...
        # 测 试 阶 段（第一帧初始化）
            ...
            self.exemplars = self.features_exemplar(exemplar)  # 1，256, 6, 6
            for i in range(len(self.exemplars)):
                self.exemplars[i] = torch.cat([self.exemplars[i] for _ in range(3)], dim=0)
            #  因为样本分支里面，包含了6个分支下的样本特征，因此要重复6次，而簇分支权重，只有一个。
            self.clu_weight = torch.cat([self.clu_weight for _ in range(3)], dim=0)
        ...
        # 测 试 阶 段 （非第一帧）
            ...
            instances = self.features_instance(instance)  # 提取搜索区域的特征，包含三个尺度
            scores = None
            for i in range(len(instances)):
                exemplar, instance = self.exemplars[i], instances[i]
...
def main():
    ...
    model_path = r'../models/'
    model = SiameseAlexNetMixCw(0, False, model_path)
    print(model)
    pass
