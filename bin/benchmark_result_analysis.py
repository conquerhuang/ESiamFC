...
def result_analysis_otb2015(result_dir):
    tracker_names = os.listdir(os.path.join(result_dir, 'OTBotb2015'))  # 获取OTB2015路径下的所有结果

    tracker_names = [x for x in tracker_names if 'siamfc_mix_cw' in x]  # 对特定的结果进行筛选
    # tracker_names = [x for x in tracker_names if '.' in x]

    tracker_results = []
    for tracker_name in tracker_names:
        tracker_dir = os.path.join(result_dir, 'OTBotb2015', tracker_name, 'performance.json')
...
def main():
    # got10k结果路径
    result_dir = r'../got10k/trackers/reports'
