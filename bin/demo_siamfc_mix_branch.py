...
def main():
    ...
    parser.add_argument('-v', '--video', default='./../video/FaceOcc1', type=str, help="the path of video directory")
    parser.add_argument('-g', '--gpuid', default=0, type=int, help="the id of GPU")
    parser.add_argument('-m', '--model', default='../models/siamfc_mix.pth', type=str, help="the path of model")
    ...
    # starting tracking
    tracker = SiamFCTrackerMixBranch(model_path, gpu_id)
    for idx, frame in enumerate(frames):
        ...
        cv2.waitKey(20)
        # 用于测试
        ious.append(iou(bbox, gt_bbox))
        if idx == (len(frames)-1):
            scio.savemat('../analysis/psr/' + video_dir.split('/')[-1]+'_psr.mat',{video_dir.split('/')[-1]+'_psr':tracker.psrs})
            scio.savemat('../analysis/psr/' + video_dir.split('/')[-1]+'_iou.mat',{video_dir.split('/')[-1]+'_iou':ious})



def iou(box1, box2):
    a1, a2, a3, a4 = box1[0], box1[1], box1[0]+box1[2], box1[1]+box1[3]
    b1, b2, b3, b4 = box2[0], box2[1], box2[0] + box2[2], box2[1] + box2[3]
    lu_x = max(a1, b1)
    lu_y = max(a2, b2)
    rd_x = min(a3, b3)
    rd_y = min(a4, b4)
    if (lu_x>rd_x) or (lu_y>rd_y):
        intersection = 0
    else:
        intersection = (lu_x - rd_x) * (lu_y - rd_y)
